...
describe('Navegação (integração)', () => {
  it('1. tocar no filme abre a tela de detalhe', async () => {
    ...
    const filmeCard = await screen.findByText('Matrix');

    fireEvent.press(filmeCard);

   const telaDetalheTitulo = await screen.findByText('Detalhes do filme');
    
    expect(telaDetalheTitulo).toBeTruthy();
  });
});
