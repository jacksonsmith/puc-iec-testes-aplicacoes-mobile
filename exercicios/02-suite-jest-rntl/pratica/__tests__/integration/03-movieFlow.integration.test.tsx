...
import '@testing-library/react-native/extend-expect'; //adicionei pq deu um BO no toHaveTextContext
...
describe('Fluxo de integração — lista + favoritos (ENTREGA Parte B)', () => {
  ...
  it('2. favoritar um filme soma no contador do topo (♥ 1)', async () => {
    render(renderApp());
    await screen.findByText('Matrix'); // espera a lista carregar
    fireEvent.press(screen.getByTestId('movie-card-heart-1'));
    expect(screen.getByTestId('favorites-count')).toHaveTextContent('1');
  });   // 🧑‍💻 aluno
  ...
  it('3. desfavoritar volta o contador a 0', async () => {
    ...
    fireEvent.press(screen.getByTestId('movie-card-heart-1')); // favorita
    fireEvent.press(screen.getByTestId('movie-card-heart-1')); // desfavorita
